...
{
  std::string::size_type n = s2.find(s1);
...
bool isRotation(const std::string s1, const std::string s2)
{
  ...
  // if the size is empty, then I've decided that it's true, since an empty
  // string is indeed a rotation of an empty string.
  if(s1.size()==0) return true;


  // Now for an example: if
  // s1 = waterbottle
  // s2 = erbottlewat
  // we can split x=wat, y=erbottle, so 
  // s1 = wat erbottle = xy
  // s2 = erbottle wat = yx
  //
  // If we concatenate, s2s2 = yxyx, we see xy in there.
  // So we can check if s1 is a substring of s2s2.
